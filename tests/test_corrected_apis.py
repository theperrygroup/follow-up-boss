...
def test_corrected_apis():
    ...
    field_types_to_try = ['text', 'number', 'date', 'dropdown', 'singlelinetext', 'multilinetext']
    
    for field_type in field_types_to_try:
        try:
            print(f'   Trying field type: {field_type}')
            result = fields_api.create_custom_field(
                name=f'Test Field {field_type} {int(time.time())}',
                type=field_type
            )
            print(f'✅ Custom Fields API Success with type "{field_type}": Created field ID {result.get("id")}')
            break
        except Exception as e:
