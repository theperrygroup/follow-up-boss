...
def test_delete_deal_custom_field(deal_custom_fields_api):
    ...
    try:
        ...
        assert delete_response == "" or delete_response == [] or (isinstance(delete_response, dict) and not delete_response) or "success" in str(delete_response).lower()
        ...
        assert excinfo.value.status_code in [404, 400, 410], f"Expected 404, 400, or 410, got {excinfo.value.status_code}"
