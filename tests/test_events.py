...
def test_create_event_with_new_person(events_api):
    ...
    try:
        ...
        print(
            f"Event with new person created successfully, person ID: {response['id']}"
        )
    except requests.exceptions.HTTPError as e:
        ...
        pytest.skip(
            f"Failed to create event with new person, API responded with error: {str(e)}"
        )
    except Exception as e:
        print(f"Error creating event with new person: {str(e)}")
        pytest.skip(
            f"Failed to create event with new person, API responded with: {str(e)}"
        )
