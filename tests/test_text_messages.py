...
def get_test_person_id(people_api):
    ...
    person_data = {
        ...
        "phones": [{"value": phone_e164, "type": "mobile"}],
    }
...
def test_list_text_messages(text_messages_api, people_api):
    ...
    assert (
        "textmessages" in response
    )  # API returns 'textmessages' (lowercase) not 'textMessages'
...
def test_create_text_message(text_messages_api, people_api):
    ...
    message = (
        f"This is a test text message created at {datetime.datetime.now().isoformat()}"
    )
    ...
    try:
        ...
        pytest.skip(
            f"Failed to create text message, API responded with error: {str(e)}"
        )
