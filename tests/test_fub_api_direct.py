...
def test_deals_api_directly() -> None:
    ...
    response = requests.get(
        f"{BASE_URL}/pipelines/{pipeline_id}", headers=headers, auth=auth
    )
    ...
    stage_only_payload = {"name": f"Stage Only Deal {timestamp}", "stageId": stage_id}

    logger.info(f"Testing stageId only payload: {stage_only_payload}")
    response = requests.post(
        f"{BASE_URL}/deals", headers=headers, auth=auth, json=stage_only_payload
    )
    ...
    response = requests.post(
        f"{BASE_URL}/deals", headers=headers, auth=auth, json=no_stage_payload
    )
    ...
    response = requests.post(
        f"{BASE_URL}/deals", headers=headers, auth=auth, json=all_fields_payload
    )
