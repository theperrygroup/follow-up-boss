...
def test_list_text_message_templates(text_message_templates_api):
    ...
    try:
        ...
        if e.response.status_code == 403:
            pytest.skip(
                f"API key doesn't have access to Text Message Templates: {str(e)}"
            )
        else:
            raise
...
def create_test_text_message_template(text_message_templates_api):
    ...
    try:
        ...
        if isinstance(response, dict) and "id" in response:
            ...
            return template_id, {"name": name, "body": body}
        else:
            pytest.skip(
                f"Failed to create test text message template, unexpected response: {response}"
            )
            return None, None
    except requests.exceptions.HTTPError as e:
        # If we get a 403, it might mean the API key doesn't have access to this endpoint
        if e.response.status_code == 403:
            pytest.skip(
                f"API key doesn't have access to create Text Message Templates: {str(e)}"
            )
        else:
            raise
...
def test_create_text_message_template(text_message_templates_api):
    ...
    try:
        ...
        if e.response.status_code in [401, 403]:
            pytest.skip(
                f"API key doesn't have permission to create templates: {str(e)}"
            )
        else:
            raise
...
def test_retrieve_text_message_template(text_message_templates_api):
    ...
    try:
        template_id, template_data = create_test_text_message_template(
            text_message_templates_api
        )
        if not template_id:
            # If we couldn't create a test template, try to get the first one from the list
            list_response = text_message_templates_api.list_text_message_templates(
                limit=1
            )
            if isinstance(list_response, dict) and (
                "textMessageTemplates" in list_response or "templates" in list_response
            ):
                templates_list = list_response.get(
                    "textMessageTemplates", list_response.get("templates", [])
                )
                if templates_list:
                ...
                pytest.skip(
                    "Could not list text message templates to find one to retrieve"
                )

        # Now retrieve the template
        response = text_message_templates_api.retrieve_text_message_template(
            template_id
        )
        ...
        if e.response.status_code in [401, 403]:
            pytest.skip(
                f"API key doesn't have permission to retrieve templates: {str(e)}"
            )
        else:
            raise
...
def test_update_text_message_template(text_message_templates_api):
    ...
    try:
        ...
        original = text_message_templates_api.retrieve_text_message_template(
            template_id
        )
        message_field = "message" if "message" in original else "body"

        update_data = {"name": updated_name, message_field: updated_body}

        # Update the template
        response = text_message_templates_api.update_text_message_template(
        ...
        if e.response.status_code in [401, 403]:
            pytest.skip(
                f"API key doesn't have permission to update templates: {str(e)}"
            )
        else:
            raise
...
def test_merge_text_message_template(text_message_templates_api):
    ...
    try:
        ...
        # Define merge fields
        merge_fields = {"firstName": "Test", "lastName": "User"}
