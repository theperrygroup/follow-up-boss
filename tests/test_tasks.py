...
def test_task_data():
    ...
    return {"name": f"Test Task {unique_suffix}"}
...
def test_list_tasks(tasks_api):
    ...
    try:
        ...
        assert (
            ...
            or "task" in metadata["collection"].lower()
        )
...
def create_test_task(tasks_api, task_data):
    ...
    try:
        ...
        print(
            f"Response content: {e.response.content if hasattr(e, 'response') else ''}"
        )
...
def test_create_and_retrieve_task(tasks_api, test_task_data):
    ...
    try:
        ...
        if e.response.status_code == 403:
            pytest.skip(
                "API key doesn't have sufficient permissions for task operations"
            )
        raise
