...
def create_test_email_template(email_templates_api):
    ...
    try:
        ...
        if isinstance(response, dict) and "id" in response:
            ...
            return template_id, {"name": name, "subject": subject, "body": body}
        else:
            pytest.skip(
                f"Failed to create test email template, unexpected response: {response}"
            )
            return None, None
    except requests.exceptions.HTTPError as e:
        # If we get a 403, it might mean the API key doesn't have access to this endpoint
        if e.response.status_code == 403:
            pytest.skip(
                f"API key doesn't have access to create Email Templates: {str(e)}"
            )
        else:
            raise
    ...
    try:
        ...
        if not template_id:
            ...
            if isinstance(list_response, dict) and (
                "templates" in list_response or "emailTemplates" in list_response
            ):
                templates_list = list_response.get(
                    "templates", list_response.get("emailTemplates", [])
                )
                if templates_list:
...
def test_merge_email_template(email_templates_api):
    ...
    try:
        ...
        if "subject" in response:
            assert (
                template_data["subject"] in response["subject"]
            )  # Original subject should be there
